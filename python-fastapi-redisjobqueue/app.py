...
def create_job(job_type: str, input_data: dict, ttl_seconds: int = 3600) -> Job:
    job_id = str(uuid4())
    job = Job(id=job_id, status=JobStatus.PENDING, type=job_type, input_data=input_data)
    ...
    redis_conn.xadd("jobs_stream", {"job_id": job_id, "type": job_type})
...
async def get_addition_result(job_id: str):
    ...
    return {"status": "complete", "result": job.result, "input": job.input_data}
