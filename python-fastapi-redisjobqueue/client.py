...
class JobClient:
    ...
    def _get_job_result(self, job_id: str) -> Optional[Dict[str, Any]]:
    ...
    def _poll_job_result(self, job_id: str) -> Dict[str, Any]:
        ...
        while attempt < self.max_attempts:
            ...
            print(f"Job {job_id} is still pending... (attempt {attempt + 1}/{self.max_attempts})")
            time.sleep(self.poll_interval)
            attempt += 1
            
        raise TimeoutError(f"Job {job_id} did not complete within {self.max_attempts} attempts")
